...
#           multiple twin approach or many devices
#
#   Online: https://github.com/Larouex/LarouexSmartKitchenOpcUaServer
#
...
# -------------------------------------------------------------------------------
async def setup_server(OpcuaServer):
...
# -------------------------------------------------------------------------------
async def load_nodes_from_devicecache(OpcuaServer):
...
# -------------------------------------------------------------------------------
async def run_server(OpcuaServer):
  ...
  try:
    ...
    await stop_server(OpcuaServer)

# -------------------------------------------------------------------------------
#   Start the OPC Server for Multiple Twin and Device Patterns
# -------------------------------------------------------------------------------
async def stop_server(OpcuaServer):
...
async def main(argv):
  ...
  # Configure Server
  opcua_server = OpcUaServer(Log)
  await setup_server(opcua_server)
  Log.info("[SERVER] Instance Info (opcua_server): %s" % opcua_server)

  # Load the meta-data and map OPC-UA to IoTC Interfaces
  await load_nodes_from_devicecache(opcua_server)

  # Start the server loop
  await run_server(opcua_server)
